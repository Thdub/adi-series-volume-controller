...
- The newer **EX series** (ADI-2 DAC EX, ADI-2 Pro EX, ADI-2/4 Pro EX) uses
  the same device IDs as their predecessors — confirmed by RME (Matthias
  Carstens, RME forum, September 2026): `71`/`72`/`73` cover both. No
  `device_id` change needed; the existing config already works. RME notes
  the ADI-2 Pro EX is functionally identical to the ADI-2 Pro for this
  purpose; the ADI-2/4 Pro EX adds digital input gain, RIAA-off and input
  impedance options over the Pro SE, none of which this app touches. SE vs.
  EX can be told apart from the status message's device-revision byte, if
  that's ever needed — not used by this app.
